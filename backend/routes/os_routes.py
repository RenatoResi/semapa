...
os_bp = Blueprint('ordens_servico', __name__, url_prefix='/ordens_servico')

# -------------------- FUNÇÕES AUXILIARES --------------------

def serializar_requerimento_basico(req):
    """Serializa dados básicos do requerimento para listagem"""
    return {
        "id": req.id,
        "numero": req.numero,
        "status": req.status,
...
def serializar_requerimento_completo(req):
    """Serializa dados completos do requerimento para detalhes"""
    return {
        "id": req.id,
        "numero": req.numero,
        "tipo": req.tipo,
...
def serializar_ordem_servico(os, completo=False):
    """Serializa ordem de serviço para JSON"""
    serializar = serializar_requerimento_completo if completo else serializar_requerimento_basico
    ...
    return {
        ...
        "requerimentos": [serializar(req) for req in os.requerimentos]
    }

# -------------------- ROTAS --------------------

@os_bp.route('/', methods=['GET', 'POST'])
@login_required
def ordens_servico():
    """Rota principal: lista (GET) ou cadastra (POST) ordens de serviço"""
