...
@especies_bp.route('/<int:id>', methods=['PUT'])
@login_required
def atualizar_especie(id):
    """Atualiza dados de uma espécie existente"""
    data = request.json
    session = SessionLocal()
    try:
...
@especies_bp.route('/api/especies_autocomplete', methods=['GET'])
@login_required
def especies_autocomplete():
    """API para autocomplete de espécies por nome popular ou científico"""
    ...
    try:
        ...
        if termo:
            ...
            query = query.filter(
                or_(
                    sa_func.lower(Especies.nome_popular).like(termo_like),
                    sa_func.lower(Especies.nome_cientifico).like(termo_like)
                )
            )
